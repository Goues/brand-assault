...
const Dialog = ({ product, level, header, ...rest }) => {
  const nextBonus = useMemo(() => PRODUCTS[product].NEXT_BONUS_DESCRIPTION(level), [product, level])
  const currentBonus = useMemo(
    () => PRODUCTS[product].CURRENT_BONUS_DESCRIPTION(level),
    [product, level]
  )
...
export default function Product({ name, product, credits, onClick }) {
  ...
  const currentBonus = useMemo(
    () => PRODUCTS[product].CURRENT_BONUS_DESCRIPTION(owned),
    [product, owned]
  )
  ...
  return (
    <div className={css.product}>
      ...
      <div className={css.actions}>
        ...
        <Tippy content={<Dialog header="Perks on next level" product={product} level={owned} />}>
