...
class Hitpoints extends PIXI.Container {
  constructor(hitpoints, maxHitpoints) {
    ...
    this.text = new PIXI.Text(hitpoints, {
      fontFamily: 'Arial',
      fill: ['#00F0F0'],
      fontSize: 35,
      fontWeight: 'bold',
    ...
    this.bar.drawRect(0, 0, HEALTHBAR_WIDTH * hitpoints / maxHitpoints, HEALTHBAR_HEIGHT)
    this.bar.endFill()
  ...
  update(delta, parent) {
    this.text.text = parent.hitpoints
    ...
    this.bar.beginFill(0x00FF00);
    this.bar.drawRect(0, 0, HEALTHBAR_WIDTH * parent.hitpoints / parent.maxHitpoints, HEALTHBAR_HEIGHT)
    this.bar.endFill()
