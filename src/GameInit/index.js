...
function SecondStep({next}) {
  return (
    <div>
      ...
      <EnemiesList />
      <div><strong>TIP</strong><br />You can deal tiny amount of damage to the enemies by clicking on them</div>
...
function FourthStep({next}) {
  return (
    <div>
      ...
      <div>
        <div>
          <div><Community /> {PRODUCTS.COMMUNITY.NAME}</div>
          <div>{PRODUCTS.COMMUNITY.DESCRIPTION}</div>
        </div>
        <div>
          <div><Publisher /> {PRODUCTS.PUBLISHER.NAME}</div>
          <div>{PRODUCTS.PUBLISHER.DESCRIPTION}</div>
        </div>
        <div>
          <div><Influencers /> {PRODUCTS.INFLUENCERS.NAME}</div>
          <div>{PRODUCTS.INFLUENCERS.DESCRIPTION}</div>
        </div>
        <div>
          <div><Audiences /> {PRODUCTS.AUDIENCES.NAME}</div>
          <div>{PRODUCTS.AUDIENCES.DESCRIPTION}</div>
        </div>
        <div>
          <div><Analytics /> {PRODUCTS.ANALYTICS.NAME}</div>
  ...
  return (
    <div className={css.wrapper}>
        <div className={css.modal}>
          ...
          {step === 4 && <LastStep next={(playerName) => {
